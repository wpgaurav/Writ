...
class writ_recent_comments extends WP_Widget {

	function __construct() {
        $widget_ops = array( 'classname' => 'widget_writ_recent_comments', 'description' => __('Displays recent comments with user avatar and excerpt.', 'writ') );
        parent::__construct( 'widget_writ_recent_comments', __('Enhanced Recent Comments','writ'), $widget_ops );
    }
	...
	function widget($args, $instance) {
		...
		if (!empty($widget_title)) {
			...
			echo $before_title . esc_html__( 'Recent Comments', 'writ' ) . $after_title;
		}
		?>
						...
						foreach ( $comments as $comment ) { ?>
							...
							<li>
								<a href="<?php echo get_permalink($comment->comment_post_ID); ?>#comment-<?php echo $comment->comment_ID; ?>">
									...
									<p class="original-title"><span><?php _e('on','writ'); ?></span> <?php the_title_attribute( array( 'post' => $comment->comment_post_ID ) ); ?></p>
								</a>
							</li>
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php  _e('Title', 'writ'); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_comments'); ?>"><?php _e('Number of comments to display', 'writ'); ?>:
...
register_widget('writ_recent_comments');
...
/*
function writ_comments_widget_registration() {
	unregister_widget('WP_Widget_Recent_Comments');
	register_widget('writ_recent_comments');
}
add_action('widgets_init', 'writ_comments_widget_registration');
*/
