...
function writ_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Published %s', 'post date', 'writ' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'writ' ),
	...
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'writ' ), esc_html__( '1 Comment', 'writ' ), esc_html__( '% Comments', 'writ' ) );
		echo '</span>';
	}
...
if ( ! function_exists( 'writ_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function writ_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between category list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'writ' ) );
		if ( $categories_list && writ_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'writ' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between tag list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'writ' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'writ' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	edit_post_link( esc_html__( 'Edit', 'writ' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function writ_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'writ_categories' ) ) ) {
		...
		set_transient( 'writ_categories', $all_the_cool_cats );
	}
...
function writ_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'writ_categories' );
}
...
function writ_paging_nav() {
	...
	$links = paginate_links( array(
		...
		'prev_text' => __( 'Previous', 'writ' ),
		'next_text' => __( 'Next', 'writ' ),
		'type'      => 'list',
	) );
...
function writ_modify_read_more_link() {
	$read_more_link = sprintf(
		/* translators: %s: Name of current post. */
		wp_kses( __( 'Continue reading%s', 'writ' ), array( 'span' => array( 'class' => array() ) ) ),
...
add_filter( 'the_content_more_link', 'writ_modify_read_more_link' );
...
if ( ! function_exists( 'writ_attachment_nav' ) ) :
/**
 * Display navigation to next/previous image in attachment pages.
 */
function writ_attachment_nav() {
	?>
	<nav class="navigation post-navigation" role="navigation">
		<div class="post-nav-box clear">
			<h1 class="screen-reader-text"><?php _e( 'Attachment post navigation', 'writ' ); ?></h1>
			<div class="nav-links">
...
function writ_the_attached_image() {
	 ...
	 */
	$attachment_size = apply_filters( 'writ_attachment_size', array( 810, 810 ) );
