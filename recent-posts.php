 ...
 * Gently lifted and reworked from Anders Norén's Lovecraft theme: http://www.andersnoren.se/teman/writ-wordpress-theme/
 */


class writ_recent_posts extends WP_Widget {

	function __construct() {
        $widget_ops = array( 'classname' => 'widget_writ_recent_posts', 'description' => __('Displays most recent posts with featured image and publishing date.', 'writ') );
        parent::__construct( 'widget_writ_recent_posts', __('Enhanced Recent Posts','writ'), $widget_ops );
    }
	...
	function widget($args, $instance) {
		...
		if (!empty($widget_title)) {
			...
			echo $before_title . esc_html__( 'Recent Posts', 'writ' ) . $after_title;
		}
		?>

			<ul class="writ-widget-list">

				<?php
					if ( $number_of_posts == 0 ) $number_of_posts = 5;

					$recent_posts = new WP_Query( apply_filters(
					'writ_recent_posts_args', array(
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('widget_title'); ?>"><?php  _e('Title', 'writ'); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_posts'); ?>"><?php _e('Number of posts to display', 'writ'); ?>:
			...
			<small>(<?php _e('Defaults to 5 if empty','writ'); ?>)</small>
		</p>

		<?php
	}
}
register_widget('writ_recent_posts');
...
/*
function writ_posts_widget_registration() {
  unregister_widget('WP_Widget_Recent_Posts');
  register_widget('writ_recent_posts');
}
add_action('widgets_init', 'writ_posts_widget_registration');
*/
